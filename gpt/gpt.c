...
/*typedef struct residual_forward_struct{
    float* out;
    float* inp1; 
    float* inp2; 
    int i;
...
void* residual_forward_thread(void *arg){
    resi* a = (resi*)arg;
    a->out[a->i] = a->inp1[a->i] + a->inp2[a->i];
    return NULL;
}
...
void residual_forward(float* out, float* inp1, float* inp2, int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        arg[i].inp1 = inp1;
        arg[i].inp2 = inp2;
        pthread_create(arr+i, NULL, residual_forward_thread, (void *)(arg+i));
    }
    for (int i=0;i<N;i++){
        pthread_join(arr[i], NULL);
    }
...
void gpt2_build_from_checkpoint(GPT2 *model, char* checkpoint_path) {
    ...
    int read_info1 = fread(model_header, sizeof(int), 256, model_file);
    read_info1++;
    ...
    int read_info2 = fread(model->params_memory, sizeof(float), num_parameters, model_file);
    read_info2++;
    fclose(model_file);
